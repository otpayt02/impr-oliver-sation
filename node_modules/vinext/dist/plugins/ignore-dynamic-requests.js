...
import { DYNAMIC_IMPORT_PRESCAN, SCRIPT_MODULE_ID_RE, collectBindingNames, forEachAstChild, hasRange, isAstRecord, isIdentifierNamed, mayContainDynamicImport, nodeArray, scriptParserLanguage, stringLiteralValue, unwrapExpression } from "./ast-utils.js";
...
function evaluateStaticString(value, scope, resolution) {
	const node = unwrapExpression(value);
	if (!node) return null;
	...
	if (node.type === "ConditionalExpression") {
		...
		if (truthiness !== null) return evaluateStaticString(truthiness ? node.consequent : node.alternate, scope, resolution);
	...
	if (node.type === "SequenceExpression") return evaluateStaticString(nodeArray(node.expressions).at(-1), scope, resolution);
	if (node.type === "Identifier" && typeof node.name === "string") return resolveConstantBinding(scope, node.name, resolution, null, evaluateStaticString);
	return null;
}
...
function stringConcatHasStaticPart(node, scope, resolution) {
	...
	const callee = unwrapExpression(node.callee);
	const property = callee?.type === "MemberExpression" ? unwrapExpression(callee.property) : null;
	if (callee?.type !== "MemberExpression" || (callee.computed === true ? property === null || evaluateStaticString(property, scope, resolution) !== "concat" : !isIdentifierNamed(property, "concat"))) return null;
