...
const appRouteRequestBuiltInsPath = resolveEntryPath("../server/app-route-request-built-ins.js", import.meta.url);
...
const appActionForwardingPath = resolveEntryPath("../server/app-action-forwarding.js", import.meta.url);
...
` : "";
	return `
${hasAppRouteHandlers ? `// Capture the canonical Request surface before any user module can extend it.
...
${actionOwners === void 0 ? `import __vinextActionOwners from ${JSON.stringify(ACTION_OWNER_MANIFEST_ID)};` : ""}
...
function __VINEXT_ACTION_OWNERS() { return ${actionOwners === void 0 ? "__vinextActionOwners" : safeJsonStringify(actionOwners)}; }
const __appRscHandler = createAppRscHandler({
  ...
  ${(metadataRoutes?.length ?? 0) > 0 ? `async getPrerenderMetadataRoutePaths() {
  ...
  ${(metadataRoutes?.length ?? 0) > 0 ? `async isMetadataRoutePath(cleanPathname) {
