...
const suppressedAbortRejections = /* @__PURE__ */ new WeakSet();
function suppressUnhandledRejection() {}
function makeHangingPromise(signal, route, expression) {
	if (signal.aborted) {
		if (suppressedAbortRejections.has(signal)) return new Promise(() => {});
	...
	const hangingPromise = new Promise((_, reject) => {
		...
		if (currentListeners) currentListeners.push(boundRejection);
			...
			signal.addEventListener("abort", () => {
				if (!suppressedAbortRejections.has(signal)) for (let i = 0; i < listeners.length; i++) listeners[i]();
				listeners.length = 0;
...
*/
function suppressHangingPromiseAbortRejections(signal) {
	suppressedAbortRejections.add(signal);
}
//#endregion
export { makeHangingPromise, suppressHangingPromiseAbortRejections };
