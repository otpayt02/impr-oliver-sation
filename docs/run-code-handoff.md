...
- Smoke check: turn Motion on, scroll the hero from top to the end of its runway, and confirm the active step changes from `input` to `book` while binary becomes notation on the right. Confirm `#services` shows four service pillars and keeps the First Listen link visible. At `#input`, choose an audio file and confirm it previews locally with a rights reminder. Then choose First Listen, complete the required fields, confirm `#book` contains the prefilled local-only brief, and click `Copy local brief`; the status should say it was copied locally and remains unsent.
...
- SEO source check: `site/index.html` contains domain-neutral `ProfessionalService` JSON-LD with the four service pillars; no canonical domain is asserted.
...
The server was started in this task and left running for review. Public hosting, contact delivery, payments, and outreach are not connected.
