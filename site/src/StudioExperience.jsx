...
export function PersonPicker({ person, onChange }) {
  return <div className="person-picker" aria-label="Preferred collaborator">{Object.entries(people).map(([key, value]) => <button key={key} type="button" aria-pressed={person === key} onClick={() => onChange(key)}><span className={`person-mark person-${key}`}>{key === 'oliver' ? 'OP' : 'AS'}</span><span>{value.name}<small>{key === 'oliver' ? 'Explore with Oliver' : 'Explore with Alexander'}</small></span><span className="person-arrow" aria-hidden="true">↗</span></button>)}</div>;
}
